...
def identificador_face():
    global melhor_nome, melhor_matrix, melhor_id
    ...
    while not detectado:
        ...
        if distancias_faces:
            ...
            if (abs((left + right) // 2 - centro_x) / raio_x) ** 2 + (
                    abs((top + bottom) // 2 - centro_y) / raio_y) ** 2 <= 1:
        ...
        cv2.imshow('Webcam_facerecognition', frame)  # Se necessário, descomente para exibir a imagem
